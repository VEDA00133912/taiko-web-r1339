class Game {
  ...
  init(controller, selectedSong, songData) {
    ...
    this.resetSection();
    this.gameLagSync =
      !this.controller.touchEnabled && !/Firefox/.test(navigator.userAgent);
  ...
  initTiming() {
    ...
    if (this.controller.calibrationMode) {
      ...
      var offsetTime =
        Math.max(
          0,
          this.timeForDistanceCircle - (firstCircle ? firstCircle.ms : 0)
        ) | 0;
    }
    if (this.controller.multiplayer) {
      ...
      var syncOffsetTime =
        Math.max(0, this.timeForDistanceCircle - syncCircles[0].ms) | 0;
  ...
  updateCirclesStatus() {
    ...
    for (var i = startIndex; i < circles.length; i++) {
      ...
      if (
        ...
        var endTime =
          circle.endTime +
          (drumrollNotes ? 0 : this.rules.bad) +
          this.controller.audioLatency;
        ...
        if (
          circle.daiFailed &&
          (ms >= circle.daiFailed.ms + this.rules.daiLeniency || ms > endTime)
        ) {
          ...
          if (!this.controller.autoPlayEnabled) {
            if (drumrollNotes) {
              ...
              if (this.controller.multiplayer === 1) {
                var value = {
                  pace:
                    (ms - circle.ms - this.controller.audioLatency) /
                    circle.timesHit,
                };
              ...
              if (branch.type === 'drumroll') {
                ...
                if (force.branch) {
                  var accuracy = Math.max(
                    0,
                    Math.min(100, branch.requirement[force.branch])
                  );
                } else {
                  var accuracy =
                    (this.sectionNotes.reduce((a, b) => a + b) /
                      this.sectionNotes.length) *
                    100;
                }
              }
  ...
  fixNoteStream(keysDon) {
    ...
    var correctNote = (circle) => {
      var type = circle.type;
      return keysDon
        ? type === 'don' || type === 'daiDon'
        : type === 'ka' || type === 'daiKa' || type === 'green';
    };
    var ms = this.elapsedTime;
    var circles = this.songData.circles;
  ...
  checkScore(circle, check) {
    ...
    var keyTime = this.controller.getKeyTime();
    var currentTime = circle.daiFailed
      ? circle.daiFailed.ms
      : keysDon
        ? keyTime['don']
        : keyTime['ka'];
    ...
    if (typeDon || typeKa || typeGreen) {
      ...
      if ((keysDon && typeDon) || (keysKa && typeKa)) {
        ...
        if (
          Math.abs(relative) >=
          (keyTimeRelative <= 25 ? this.rules.bad : this.rules.good)
        ) {
          return true;
        }
      ...
      if (
        circle.ms + this.controller.audioLatency > currentTime ||
        currentTime > circle.endTime + this.controller.audioLatency
      ) {
        return true;
      }
      if ((keysDon || keyGreen) && type === 'balloon') {
        ...
        (keysDon || keysKa || keyGreen) &&
        (type === 'drumroll' || type === 'daiDrumroll')
      ) {
  ...
  checkBalloon(circle) {
    if (circle.timesHit >= circle.requiredHits - 1) {
      ...
      if (this.controller.multiplayer == 1) {
        p2.send('drumroll', {
          pace:
            (this.elapsedTime - circle.ms + this.controller.audioLatency) /
            circle.timesHit,
        });
      }
    } else {
  ...
  whenLastCirclePlayed() {
    ...
    if (
      !this.fadeOutStarted &&
      ms >= this.lastCircle + 2000 + this.controller.audioLatency
    ) {
    ...
    if (started) {
      ...
      if (this.musicFadeOut === 0) {
        ...
        ms >=
          Math.max(
            started + 8600,
            Math.min(started + 8600 + 5000, musicDuration + 250)
          )
      ) {
        ...
        ms >=
          Math.max(
            started + 9600,
            Math.min(started + 9600 + 5000, musicDuration + 1250)
          )
      ) {
  ...
  playMainMusic() {
    ...
    if (
      !this.mainMusicPlaying &&
      (!this.fadeOutStarted || ms < this.fadeOutStarted + 1600)
    ) {
    ...
    if (!this.paused) {
      ...
      if (
        ...
        [
          ...
          'videoComplete',
          'results',
        ].indexOf(this.calibrationState) !== -1
      ) {
        return;
      }
      if (
        this.calibrationState === 'audioHelp' ||
        this.calibrationState === 'videoHelp'
      ) {
        this.calibrationState =
          this.calibrationState === 'audioHelp' ? 'audio' : 'video';
  ...
  updateCurrentCircle() {
    var circles = this.songData.circles;
    if (
      circles[this.currentCircle] &&
      circles[this.currentCircle].type === 'adlib'
    ) {
  ...
  updateGlobalScore(score, multiplier, gogoTime) {
    ...
    if (this.songData.scoremode == 2) {
      ...
      score += Math.max(
        0,
        Math.floor((Math.min(this.combo, 100) - 1) / 10) *
          (this.songData.scoremode ? this.songData.scorediff : 100)
      );
    }
  ...
  setBranch(currentBranch, activeName) {
    ...
    for (var i = 0; i < this.songData.branches.length; i++) {
      ...
      if (branch.ms >= ms) {
        ...
        var noAdvanced =
          req.master <= 0 ||
          req.advanced >= req.master ||
          (branch.type === 'accuracy' && req.advanced > 100);
    ...
    if (!circle || circle.branch === currentBranch[pastActive]) {
      ...
      var closestCircle = circles.findIndex((circle) => {
        return (
          (!circle.branch || circle.branch.active) &&
          circle.endTime + this.controller.audioLatency >= ms
        );
      });
      if (closestCircle !== -1) {
    ...
    switch (state) {
      ...
      case 'video':
        ...
        if (progress.hit >= progress.requirement) {
          ...
          progress[state] = Math.max(
            0,
            Math.round(reduced / progress.offsets.length - 2)
          );
