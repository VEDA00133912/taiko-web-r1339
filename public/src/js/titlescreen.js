class Titlescreen {
  ...
  init(songId) {
    ...
    if (!songId) {
      ...
      this.disclaimerCopyright = document.getElementById(
  ...
  onPressed(pressed, name) {
    if (pressed) {
      if (
        name === 'gamepadConfirm' &&
        (snd.buffer.context.state === 'suspended' || this.customFolder)
      ) {
        return;
      }
      ...
      if (
        ...
        if (this.customFolder && !fromP2 && !assets.customSongs) {
          ...
          this.customFolder.forEach((file) => {
            promises.push(
              customSongs.walkFilesystem(file, undefined, allFiles)
            );
          });
