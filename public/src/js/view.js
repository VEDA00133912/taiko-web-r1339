class View {
  ...
  refresh() {
    ...
    var touchMultiplayer =
      this.touchEnabled && this.multiplayer && !this.portrait;
      ...
      this.titleCache.get(
        ...
        (ctx) => {
          ...
          if (selectedSong.category) {
            ...
            let category = assets.categories.find(
              (cat) => cat.id == selectedSong.category_id
            );
            if (
              category != null &&
              category.songSkin != null &&
              category.songSkin.infoFill != null
            ) {
              ctx.fillStyle = category.songSkin.infoFill;
            } else {
              ctx.fillStyle = assets.categories.find(
                (cat) => cat.title == 'default'
              ).songSkin.infoFill;
            }
    ...
    if (this.portrait) {
      ...
      if (!p2.session || p2.player === 1) {
        ...
        var rank = account.loggedIn
          ? account.rank && account.rank.rank_name
            ? account.rank.rank_name
            : 'ドンだーデビュー！'
      ...
      if (this.controller.selectedSong.difficulty) {
        ctx.drawImage(
          ...
          0,
          144 * this.difficulty[this.controller.selectedSong.difficulty],
          168,
          143,
          126,
          this.player === 2 ? 497 : 228,
          62,
          53
        );
      }
    ...
    var touchTop =
      frameTop + (touchMultiplayer ? 135 : 0) + (this.player === 2 ? -165 : 0);
    this.touchDrum = (() => {
      var sw = 842;
      var sh = 340;
      var x = 0;
    ...
    if (this.branch) {
      ...
      if (
        this.branchAnimate &&
        ms - this.branchAnimate.ms < 310 &&
        ms >= this.branchAnimate.ms
      ) {
        var fromBranch = this.branchAnimate.fromBranch;
        var elapsed = ms - this.branchAnimate.ms;
        var reverse =
          fromBranch === 'master' ||
          (fromBranch === 'advanced' && this.branch === 'normal')
            ? -1
            : 1;
        if (elapsed < 65) {
    ...
    if (keyTime[sound] > ms - 130) {
      ...
      do {
        ...
        this.currentScore.ms > ms - 130 &&
        currentGradient !== yellow &&
        (currentGradient = yellow)
      );
    ...
    ctx.beginPath();
    ctx.rect(
      this.slotPos.paddingLeft,
      0,
      winW - this.slotPos.paddingLeft,
      winH
    );
    ctx.clip();
    this.drawMeasures();
    ...
    if (scoreMS < 300) {
      ...
      var drawScore = this.currentScore.adlib
        ? 'adlib'
        : scores[this.currentScore.type];
    ...
    if (!this.multiplayer && this.controller.game.paused) {
      ...
      switch (state) {
        ...
        case 'results':
          var content =
            state === 'audioHelp' && this.touchEnabled
              ? 'contentAlt'
              : 'content';
          if (state === 'audioHelp') {
            ...
            var substitute = (config, index, width) => {
              ...
              ctx.font =
                bold + config.fontSize * 0.66 + 'px ' + config.fontFamily;
              var w =
                config.fontSize * 0.6 + ctx.measureText(keys[index]).width;
              if (width) {
                return w;
              } else {
                var h = 30;
            ...
            var latency = [progress.audio, progress.video];
            var substitute = (config, index, width) => {
              var ctx = config.ctx;
              var bold = this.draw.bold(config.fontFamily);
              ctx.font =
                bold + config.fontSize * 1.1 + 'px ' + config.fontFamily;
        ...
        case 'videoComplete':
          ...
          break;
        default:
          ctx.drawImage(assets.image['mimizu'], 313, 247, 136, 315);
          ...
          for (var i = 0; i < this.pauseOptions.length; i++) {
            var text = this.pauseOptions[i];
            if (
              this.controller.calibrationMode &&
              i === this.pauseOptions.length - 1
            ) {
  ...
  setBackground() {
    ...
    if (!selectedSong.songSkin.song) {
      ...
      this.setLayers(
        songLayers,
        prefix + 'bg_song_' + songSkinName + (notStatic ? '_' : ''),
        notStatic
      );
    }
    ...
    if (!selectedSong.songSkin.stage) {
      ...
      this.setBgImage(
        this.songStage,
        assets.image['bg_stage_' + selectedSong.songStage].src
      );
    } else if (selectedSong.songSkin.stage !== 'none') {
      var prefix = selectedSong.songSkin.prefix || '';
      this.setBgImage(
        this.songStage,
        assets.image[prefix + 'bg_stage_' + songSkinName].src
      );
    }
  }
  setDonBg() {
    ...
    var donLayers = [];
    var filename =
      !selectedSong.songSkin.don && this.player === 2 ? 'bg_don2_' : 'bg_don_';
    var prefix = '';
    ...
    if (!selectedSong.songSkin.don) {
      ...
      this.setLayers(
        donLayers,
        prefix + filename + songSkinName + (notStatic ? '_' : ''),
        notStatic
      );
    } else {
      return;
  ...
  drawCircles(circles) {
    ...
    for (var i = circles.length; i--; ) {
      ...
      var startingTime =
        circle.ms - timeForDistance + this.controller.videoLatency;
      var finishTime =
        circle.endTime +
        this.posToMs(
          this.slotPos.x - this.slotPos.paddingLeft + this.slotPos.size * 2,
          speed
        ) +
        this.controller.videoLatency;
    ...
    for (var i = 0; i < game.songData.events.length; i++) {
      ...
      if (
        ms - this.controller.audioLatency >= event.ms &&
        !event.beatMSCopied &&
        (!event.branch || event.branch.active)
      ) {
    ...
    for (var i = 0; i < circles.length; i++) {
      ...
      if (circle.animating) {
        ...
        if (ms < animT + 490) {
          ...
          var bezierPoint = this.calcBezierPoint(
            this.draw.easeOut(animPoint),
            this.animateBezier
          );
  ...
  fillBranchCache() {
    ...
    var textY = 63 * mul;
    var fontSize =
      (strings.id === 'en' ? 33 : strings.id === 'ko' ? 38 : 43) * mul;
  ...
  drawGogoTime() {
    ...
    if (this.gogoTime) {
      ...
      if (
        animation === 'gogo' ||
        (cleared && animation === 'normal') ||
        (!cleared && animation === 'clear')
      ) {
  ...
  drawTouch() {
    if (this.touchEnabled) {
      ...
      if (
        drumHeight !== this.touchDrumHeight ||
        drumWidth !== this.touchDrumWidth
      ) {
  ...
  mouseOffset(offsetX, offsetY) {
    return {
      x:
        (offsetX * this.pixelRatio - this.winW / 2) / this.ratio +
        (this.portrait ? 720 : 1280) / 2,
      y:
        (offsetY * this.pixelRatio - this.winH / 2) / this.ratio +
        (this.portrait ? 1280 : 720) / 2,
    };
  }
  pointer(enabled) {
    if (!this.canvas) {
  ...
  pauseMouse(x, y) {
    ...
    switch (this.controller.game.calibrationState) {
      ...
      case 'results':
        if (
          554 - 90 * this.pauseOptions.length <= y &&
          y <= 554 &&
          404 <= x &&
          x <= 876
        ) {
      ...
      default:
        if (
          ...
          y <= 575 &&
          465 <= x &&
          x <= 465 + 110 * this.pauseOptions.length
        ) {
