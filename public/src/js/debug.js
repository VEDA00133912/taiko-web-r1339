class Debug {
  ...
  init() {
    ...
    this.branchResetBtn =
      this.branchSelectDiv.getElementsByClassName('reset')[0];
    ...
    pageEvents.add(
      window,
      ['mousedown', 'mouseup', 'touchstart', 'touchend', 'blur', 'resize'],
  ...
  updateStatus() {
    if (debugObj.controller && !this.controller) {
      ...
      if (this.songHash === selectedSong.hash) {
        ...
        this.lyricsSlider.set(
          this.controller.lyrics ? this.controller.lyrics.vttOffset / 1000 : 0
        );
      }

      var measures = this.controller.parsedSongData.measures.filter(
        (measure, i, array) => {
...
class InputSlider {
  ...
  init(sliderDiv, min, max, fixedPoint) {
    ...
    pageEvents.add(
      window,
      ['mouseup', 'touchstart', 'touchend', 'blur'],
      (event) => {
