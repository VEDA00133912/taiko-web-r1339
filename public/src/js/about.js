class About {
  ...
  addDiag() {
    ...
    diag.push(
      'Blur performance: ' + perf.blur + 'ms, all images: ' + perf.allImg + 'ms'
    );
    ...
    diag.push(
      'Audio Latency: ' +
        (latency.audio > 0 ? '+' : '') +
        latency.audio.toString() +
        'ms, Video Latency: ' +
        (latency.video > 0 ? '+' : '') +
        latency.video.toString() +
        'ms'
    );
    var pluginList = plugins.allPlugins.map((pluginLoader) => {
      return (
        ((pluginLoader.plugin.module && pluginLoader.plugin.module.name) ||
          pluginLoader.name) + (pluginLoader.plugin.started ? ' (started)' : '')
      );
    });
    ...
    if (this.hasEmail) {
      this.getLink(this.linkEmail).href +=
