class Gpicker {
            ...
            this.displayPicker((data) => {
              if (data.action === 'picked') {
                ...
                var walk = (files, output = []) => {
                  ...
                  for (
                    var i = 0;
                    i < folders.length && batchList.length < 100;
                    i++
                  ) {
                    if (!folders[i].listed) {
                      folders[i].pos = i;
                  ...
                  if (batchList.length) {
                    ...
                    batchList.forEach((folder) => {
                      ...
                      batch.add(gapi.client.drive.files.list(req), {
                        id: folder.pos,
                      });
                    });
                      ...
                      .then(() =>
                        batch.then((responses) => {
                          ...
                          for (var i in responses.result) {
                            ...
                            if (result.error) {
                              if (
                                result.error.errors[0].domain !== 'usageLimits'
                              ) {
                                console.warn(result);
                              } else if (!rateLimited) {
                                rateLimited = true;
                          ...
                          if (rateLimited) {
                            return this.sleep(
                              Math.pow(2, rateLimit) * 1000
                            ).then(() => walk(files, output));
                          } else {
  ...
  checkScope() {
    return google.accounts.oauth2.hasGrantedAnyScope(
      this.tokenResponse,
      this.scope
    );
  }
  switchAccounts(lockedCallback, errorCallback) {
    return this.loadApi().then(() =>
      this.getToken(lockedCallback, errorCallback, true)
    );
  }
