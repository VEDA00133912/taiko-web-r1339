class P2Connection {
  ...
  open() {
    if (this.closed && !this.disabled) {
      ...
      this.socket = new WebSocket(
        gameConfig.multiplayer_url
          ? gameConfig.multiplayer_url
          : wsProtocol + '//' + location.host + location.pathname + 'p2'
      );
    ...
    switch (response.type) {
      ...
      case 'gameresults':
        ...
        for (var i in response.value) {
          this.results[i] =
            response.value[i] === null ? null : response.value[i].toString();
        }
        break;
      case 'note':
  ...
  play(circle, mekadon) {
    if (this.otherConnected || this.notes.length > 0) {
      ...
      if (drumrollNotes) {
        mekadon.playDrumrollAt(
          circle,
          0,
          this.drumrollPace,
          type === 'drumroll' || type === 'daiDrumroll' ? this.kaAmount : 0
        );
