class ImportSongs {
  ...
  init(limited, otherFiles, noPlugins, pluginAmount) {
    ...
    this.comboVoices = ['v_combo_50'].concat(
      Array.from(Array(50), (d, i) => 'v_combo_' + (i + 1) * 100)
    );
  }
  load(files) {
    var extensionRegex = /\.[^\/]+$/;
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (name.endsWith('.tja') || name.endsWith('.tjf')) {
        ...
        (!this.limited && (name === 'genre.ini' || name === 'box.def')) ||
        name === 'songtitle.txt'
      ) {
        ...
        !this.limited &&
        (path.indexOf('/taiko-web assets/') !== -1 ||
          path.indexOf('taiko-web assets/') === 0)
      ) {
    ...
    if (!this.noPlugins && this.pluginFiles.length) {
      ...
      this.pluginFiles.forEach((fileObj) => {
        pluginPromises.push(
          this.addPlugin(fileObj).catch((e) => console.warn(e))
        );
      });
      return Promise.all(pluginPromises).then(() => {
        ...
        if (
          this.plugins.length &&
          confirm(
            strings.plugins.warning.replace(
              '%s',
              strings.plugins.plugin[
                strings.plural.select(this.plugins.length)
              ].replace('%s', this.plugins.length.toString())
            )
          )
        ) {
  ...
  addMeta(fileObj) {
    ...
    var name = file.name.toLowerCase();
    return file
      .read(name === 'songtitle.txt' ? undefined : 'utf-8')
      .then((data) => {
        ...
        if (name === 'genre.ini') {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (line.startsWith('[') && line.endsWith(']')) {
              ...
              if (
                equalsPos !== -1 &&
                line.slice(0, equalsPos).trim() === 'genrename'
              ) {
        ...
        if (category) {
          ...
          var filesLoop = (fileObj) => {
            var tjaPath = fileObj.file.path
              .toLowerCase()
              .slice(0, fileObj.file.name.length * -1);
            if (
              tjaPath.startsWith(metaPath) &&
              (!('categoryLevel' in fileObj) || fileObj.categoryLevel < level)
            ) {
              if (category.toLowerCase() in this.categoryAliases) {
                fileObj.category_id =
                  this.categoryAliases[category.toLowerCase()];
              } else {
  ...
  addTja(fileObj) {
    ...
    if (!this.limited) {
      var filePromise = file.read(
        prompt(
          '太鼓さん次郎のファイルは"sjis"、TJAPlayer3のファイルは"utf-8"と入力してください。'
        )
      );
    } else {
    ...
    return filePromise.then((dataRaw) => {
      ...
      for (var diff in tja.metadata) {
        ...
        songObj.title =
          meta.title || file.name.slice(0, file.name.lastIndexOf('.'));
        ...
        if (meta.wave) {
          songObj.music =
            this.otherFiles[dir + meta.wave.toLowerCase()] || songObj.music;
        }
        if (meta.genre) {
          if (meta.genre.toLowerCase() in this.categoryAliases) {
            songObj.category_id =
              this.categoryAliases[meta.genre.toLowerCase()];
          } else {
        ...
        for (var id in allStrings) {
          ...
          if (meta['title' + id]) {
            ...
            songTitle in this.songTitle &&
            this.songTitle[songTitle][id]
          ) {
      ...
      if (!songObj.category_id && !songObj.category) {
        if (!category && category_id === undefined) {
          songObj.category_id = this.getCategory(file, [
    ...
    return filePromise.then((dataRaw) => {
      ...
      var songObj = {
        ...
        preview: osu.generalInfo.PreviewTime
          ? osu.generalInfo.PreviewTime / 1000
          : 0,
        courses: {},
        music:
          (osu.generalInfo.AudioFilename
            ? this.otherFiles[dir + osu.generalInfo.AudioFilename.toLowerCase()]
            : '') || 'muted',
      };
      if (!this.limited) {
        songObj.courses.oni = {
      ...
      if (title) {
        ...
        songObj.title_lang = {
          en:
            (osu.metadata.Title || osu.metadata.TitleUnicode || title) + suffix,
        };
      } else {
  ...
  addAssets() {
    ...
    for (let name in this.assetFiles) {
      ...
      if (assets.audioSfxLR.indexOf(name) !== -1) {
        ...
        promises.push(
          this.loadSound(file, name, snd.sfxGain).then((sound) => {
      ...
      if (this.comboVoices.indexOf(id) !== -1) {
        promises.push(
          snd.sfxGain.load(file).then((sound) => {
  ...
  getSkin(dir, config) {
    ...
    for (var i in configArray) {
      ...
      if (space !== -1) {
        configObj[string.slice(0, space).trim()] = string
          .slice(space + 1)
          .trim();
      }
    }
  ...
  loaded() {
    ...
    if (this.songs.length) {
      ...
      if (
        (this.noPlugins && this.pluginAmount) ||
        Object.keys(this.assetFiles).length
      ) {
