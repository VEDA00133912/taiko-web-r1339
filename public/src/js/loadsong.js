  ...
  run() {
    ...
    if (song.songSkin && song.songSkin.name) {
      ...
      for (var i = 0; i < imgLoad.length; i++) {
        ...
        if (songObj.custom) {
          this.addPromise(
            song.songSkin[filename + '.png'].blob().then((blob) => {
    ...
    if (songObj.sound && songObj.sound.buffer) {
      ...
      this.addPromise(
        snd.musicGain.load(songObj.music).then((sound) => {
          songObj.sound = sound;
        }),
        songObj.music.url
      );
    ...
    if (chart) {
      this.addPromise(
        chart.read(song.type === 'tja' ? 'utf-8' : '').then((data) => {
