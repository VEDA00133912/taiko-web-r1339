class SongSelect {
  ...
  init(fromTutorial, fadeIn, touchEnabled, songId, showWarning) {
    ...
    for (let song of assets.songs) {
      ...
      song.titlePrepared = title
        ? fuzzysort.prepare(this.search.normalizeString(title))
        : null;
      var subtitle = this.getLocalTitle(
        title === song.title ? song.subtitle : '',
        song.subtitle_lang
      );
      song.subtitlePrepared = subtitle
        ? fuzzysort.prepare(this.search.normalizeString(subtitle))
        : null;
    ...
    var newSelected = -1;
    if (fromTutorial) {
      newSelected = this.songs.findIndex(
    ...
    if (newSelected !== -1) {
      ...
      if (songIdIndex !== -1) {
        ...
        this.setSelectedSong(
          Math.min(Math.max(0, assets.customSelected), this.songs.length - 1),
          false
        );
  ...
  keyPress(pressed, name, event, repeat) {
    if (pressed) {
      if (!this.pressedKeys[name]) {
        this.pressedKeys[name] =
          this.getMS() + (name === 'left' || name === 'right' ? 150 : 300);
      }
    } else {
    ...
    var ctrl = event
      ? event.ctrlKey
      : this.pressedKeys['ctrl'] || this.pressedKeys['ctrlGamepad'];
  ...
  diffSelMouse(x, y) {
    if (this.state.locked === 0) {
      if (
        223 < x &&
        x < 223 + 72 * this.diffOptions.length &&
        132 < y &&
        y < 436
      ) {
  ...
  toSelectDifficulty(fromP2, playVoice = true) {
    ...
    if (
      p2.session &&
      !fromP2 &&
      (!currentSong.action || !currentSong.p2Enabled)
    ) {
      if (this.songs[this.selectedSong].courses) {
  ...
  toSongSelect(fromP2) {
    if (p2.session && !fromP2) {
      if (!this.state.selLock) {
        this.state.selLock = true;
        p2.send('songsel', {
          song: this.lastRandom
            ? this.songs.findIndex((song) => song.action === 'random')
            : this.selectedSong,
        });
      }
  ...
  toOptions(moveBy) {
    if (!p2.session) {
      ...
      do {
        ...
        (!p2.socket || p2.socket.readyState !== 1 || assets.customSongs) &&
  ...
  redraw() {
      ...
      if (this.pressedKeys[key]) {
        if (
          ...
          this.pressedKeys[key] +
            (this.state.screen === 'song' && (key === 'right' || key === 'left')
              ? 20
              : 50)
        ) {
    ...
    if (screen === 'song') {
      ...
      var resize =
        changeSpeed *
        (lastMoveMul === 0 ? 0 : this.songSelecting.resize / lastMoveMul);
      var scrollDelay = changeSpeed * this.songSelecting.scrollDelay;
      var resize2 = changeSpeed - resize;
      var scroll = resize2 - resize - scrollDelay * 2;
      ...
      if (
        ...
        if (!isJump) {
          ...
          this.setSelectedSong(
            this.mod(this.songs.length, this.selectedSong + this.state.move)
          );
        } else {
  ...
  startPreview(loadOnly) {
    if (
      (!loadOnly && this.state && this.state.showWarning) ||
      this.state.waitPreview > this.getMS()
    ) {
      return;
    }
    var currentSong = this.songs[this.selectedSong];
  ...
  getUnloaded(selectedSong, songObj, currentId) {
    ...
    return file
      .read(currentSong.type === 'tja' ? 'utf-8' : '')
      .then((data) => {
  ...
  onsongsel(response) {
    if (response && response.value) {
      ...
      if (
        'fromRandom' in response.value &&
        response.value.fromRandom === true
      ) {
        this.lastRandom = true;
      }
    ...
    if (titleLang) {
      for (var id in titleLang) {
        if (
          (id === 'en' &&
            strings.preferEn &&
            !(strings.id in titleLang) &&
            titleLang.en) ||
          (id === strings.id && titleLang[id])
        ) {
          return titleLang[id];
        }
      }
