class Scoresheet {
    ...
    if (elapsed >= 0) {
      ...
      this.draw.alpha(Math.min(1, elapsed / 400), ctx, (ctx) => {
        ...
        for (var p = 0; p < players; p++) {
          ...
          let badge_names = [this.controller.getModBadge()].flat();
          ...
          badge_names.forEach((badge, index) => {
            const badgeImage = assets.image[badge];
            
            if (badgeImage) {
              console.log([badgeImage, badge]);
              ctx.drawImage(
                badgeImage,
        ...
        for (var i in printNumbers) {
          ...
          for (var p = 0; p < players; p++) {
            ...
            if (printNumbers[i] === 'adlib') {
              var resultsNumber = (
                results.adlibTotal > 0
                  ? Math.floor((results.adlib / results.adlibTotal) * 100)
                  : 0
              ).toString();
            } else {
            ...
            var currentTime =
              lastTime + 500 + resultsNumber.length * 30 * this.frame;
  ...
  saveScore() {
    if (this.controller.saveScore) {
      ...
      if (clearReached) {
        crown =
          this.resultsObj.bad === 0
            ? this.resultsObj.ok === 0
              ? 'rainbow'
              : 'gold'
            : 'silver';
      }
      if (!oldScore || oldScore.points <= this.resultsObj.points) {
        if (
          oldScore &&
          (oldScore.crown === 'rainbow' ||
            (oldScore.crown === 'gold' && (crown === 'silver' || !crown)) ||
            (oldScore.crown === 'silver' && !crown))
        ) {
        ...
        oldScore &&
        ((crown === 'rainbow' && oldScore.crown !== 'rainbow') ||
          (crown === 'gold' &&
            (oldScore.crown === 'silver' || !oldScore.crown)) ||
          (crown && !oldScore.crown))
      ) {
