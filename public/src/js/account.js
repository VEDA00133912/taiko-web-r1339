class Account {
  ...
  accountForm() {
    ...
    parent.insertBefore(
      document.createTextNode(strings.account.rank.rankcolor),
      parent.firstChild
    );
    ...
    this.frames = [
      0, 0, 0, 0, 1, 2, 3, 4, 5, 6, 6, 5, 4, 3, 2, 1, 0, 0, 0, 0, 1, 2, 3, 4, 5,
      6, 6, 5, 4, 3, 2, 1, 0, 0, 0, 0, 1, 2, 3, 4, 5, 6, 6, 5, 7, 8, 9, 10, 11,
      11, 11, 11, 10, 9, 8, 7, 13, 12, 12, 13, 14, 15, 16, 17,
    ];
    ...
    pageEvents.add(
      this.customdonCanvas,
      'click',
      this.customdonPause.bind(this)
    );
    ...
    parent.insertBefore(
      document.createTextNode(strings.account.customdon.bodyFill),
      parent.firstChild
    );
    pageEvents.add(
      this.customdonBodyFill,
      ['change', 'input'],
      this.customdonChange.bind(this)
    );
    ...
    parent.insertBefore(
      document.createTextNode(strings.account.customdon.faceFill),
      parent.firstChild
    );
    pageEvents.add(
      this.customdonFaceFill,
      ['change', 'input'],
      this.customdonChange.bind(this)
    );
  ...
  customdonRedraw() {
    ...
    if (this.redrawPaused) {
      ...
      var frame =
        this.frames[Math.floor((ms - this.start) / 30) % this.frames.length];
    }
    var w = 360;
    var h = 184;
    ...
    ctx.drawImage(
      assets.image['don_anim_normal_a'],
      sx,
      sy,
      w,
      h,
  ...
  loginForm(register, fromSwitch) {
    ...
    this.getElement('remember-label').appendChild(
      document.createTextNode(strings.account.remember)
    );
  ...
  onLogin(event) {
    ...
    if (!obj.username || !obj.password) {
      this.error(
        strings.account.cannotBeEmpty.replace(
          '%s',
          strings.account[!obj.username ? 'username' : 'password']
        )
      );
      return;
    }
    ...
    this.request(this.mode, obj).then(
      (response) => {
        ...
        if (this.mode === 'login') {
          this.request('scores/get', false, true).then(
            (response) => {
  ...
  onSave(event) {
    ...
    if (!noNameChange && newName !== account.displayName) {
      promises.push(
        this.request('account/display_name', {
    ...
    if (
      !noNameChange &&
      (newRank !== (account.rank?.rank_name || '') ||
        rankColor !== (account.rank?.rank_color || '#ecb158'))
    ) {
      promises.push(
    ...
    if (
      !noNameChange &&
      (bodyFill !== account.body_fill ||
        this.customdonFaceFill.value !== account.face_fill)
    ) {
      promises.push(
        this.request('account/don', {
          body_fill: bodyFill,
          face_fill: faceFill,
        }).then((response) => {
          account.don = response.don;
  ...
  clean(eventsOnly, noReset) {
    ...
    if (this.mode === 'account') {
      ...
      for (var i = 0; i < this.form.length; i++) {
        pageEvents.remove(this.registerButton, [
          'keydown',
          'keyup',
          'keypress',
        ]);
