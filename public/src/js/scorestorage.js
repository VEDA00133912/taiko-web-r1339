class ScoreStorage {
  ...
  writeString(hash) {
    ...
    for (var i = this.difficulty.length; i--; ) {
      ...
      if (score[diff]) {
        ...
        var crown =
          score[diff].crown === 'rainbow'
            ? '2'
            : this.crownValue.indexOf(score[diff].crown).toString();
  ...
  getP2(song, difficulty, isHash) {
    if (!song) {
      ...
      var hash = isHash ? song : this.titleHash(song);
      if (
        !(hash in this.scoresP2) &&
        !this.requestP2.has(hash) &&
        !this.requestedP2.has(hash)
      ) {
  ...
  add(song, difficulty, scoreObject, isHash, setTitle, saveFailed) {
    ...
    if (saveFailed) {
      ...
      return this.sendToServer({
        ...
        this.add(song, difficulty, scoreObject, isHash, setTitle, true)
      );
    }
  }
