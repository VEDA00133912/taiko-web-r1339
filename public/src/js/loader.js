class Loader {
  ...
  init(callback) {
    ...
    this.errorMessages = [];
    this.songSearchGradient =
      'linear-gradient(to top, rgba(245, 246, 252, 0.08), #ff5963), ';
  ...
  run() {
    ...
    Promise.all(this.promises).then(() => {
      ...
      this.addPromise(
        this.ajax('api/songs').then((songs) => {
          ...
          songs.forEach((song) => {
            ...
            if (song.preview > 0) {
              song.previewMusic = new RemoteFile(
                directory + 'preview.' + gameConfig.preview_type
              );
            }
          });
      ...
      if (gameConfig.accounts) {
        this.addPromise(
          this.ajax('api/scores/get').then((response) => {
    ...
    if (!this.error) {
      ...
      if (typeof allStrings === 'object') {
        ...
        loader.screen.getElementsByClassName('view-content')[0].innerText =
          (allStrings[lang] && allStrings[lang].errorOccured) ||
          allStrings.en.errorOccured;
      }
      var loaderError =
