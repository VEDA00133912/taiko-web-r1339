class CustomSongs {
  ...
  init(touchEnabled, noPage, noLoading) {
    ...
    if (gameConfig.google_credentials.gdrive_enabled) {
      ...
      pageEvents.add(
        this.linkGdriveFolder,
        ['mousedown', 'touchstart'],
        this.gdriveFolder.bind(this)
      );
      ...
      this.setAltText(
        this.linkGdriveAccount,
        strings.customSongs.gdriveAccount
      );
      pageEvents.add(
        this.linkGdriveAccount,
        ['mousedown', 'touchstart'],
        this.gdriveAccount.bind(this)
      );
    ...
    pageEvents.add(this.errorEnd, ['mousedown', 'touchstart'], () =>
      this.hideError(true)
    );
  ...
  localFolder(event) {
    ...
    if (
      typeof showDirectoryPicker === 'function' &&
      !/\bOPR\/|\bOPRGX\//.test(navigator.userAgent)
    ) {
      return showDirectoryPicker().then(
        (file) => {
          this.walkFilesystem(file)
            .then((files) => this.importLocal(files))
            .then((input) => {
              if (input) {
  ...
  walkFilesystem(file, path = '', output = []) {
    if (file.kind === 'directory') {
      return filePermission(file).then(
        (file) => {
          var values = file.values();
          var walkValues = () =>
            values.next().then((generator) => {
              if (generator.done) {
                return output;
              }
              return this.walkFilesystem(
                generator.value,
                path + file.name + '/',
                output
              ).then(() => walkValues());
  ...
  filesDropped(event) {
    ...
    Promise.all(dropPromises)
      .then(() => this.importLocal(allFiles))
      .then((input) => {
      ...
      if (entry.isFile) {
        ...
        dirReader.readEntries((entries) => {
          ...
          for (var i = 0; i < entries.length; i++) {
            dirPromises.push(
              this.walkEntry(entries[i], path + entry.name + '/', output)
            );
          }
  ...
  songsLoaded(songs) {
    if (songs) {
      ...
      assets.customSelected = this.noPage
        ? +localStorage.getItem('customSelected')
        : 0;
    }
    if (this.noPage) {
