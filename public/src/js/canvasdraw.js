class CanvasDraw {
  ...
  songFrame(config) {
    ...
    if (config.cached) {
      if (
        this.songFrameCache.w !== config.frameCache.w ||
        this.songFrameCache.scale !== config.frameCache.ratio
      ) {
        this.songFrameCache.resize(
          config.frameCache.w,
          config.frameCache.h,
          config.frameCache.ratio
        );
      }
  ...
  highlight(config) {
    ...
    if (config.animate) {
      ctx.globalAlpha = this.fade(
        ((this.getMS() - config.animateMS) % 2000) / 2000
      );
  ...
  verticalText(config) {
    ...
    if (config.selectable) {
      ...
      style.left =
        (config.x - config.width / 2) * scale +
        (config.selectableX || 0) +
        'px';
    ...
    if (height && drawnHeight > height) {
      ...
      if (config.selectable) {
        ...
        style.top =
          (config.y + (height - drawnHeight) / 2 - (15 / 2) * scaling) * scale +
          'px';
      }
    }
    ...
    for (let action of actions) {
      ...
      for (let symbol of drawn) {
        ...
        offsetY =
          offsetY + symbol.h * mul * (config.align === 'bottom' ? -1 : 1);
        if (action === 'selectable') {
          ...
          if (symbol.scale) {
            transform.push(
              'scale(' + symbol.scale[0] + ',' + symbol.scale[1] + ')'
            );
          }
  ...
  layeredText(config, layers) {
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      if (symbol === '-') {
        ...
        // Exclamation mark
        var nextExclamation = string[i + 1]
          ? r.exclamation.test(string[i + 1])
          : false;
        drawn.push({
          text: symbol,
    ...
    for (let layer of layers) {
      ...
      for (let symbol of drawn) {
        var saved = false;
        var currentX =
          offsetX + symbol.x * mul + (layer.x || 0) + (symbol.w * mul) / 2;
  ...
  diffIcon(config) {
    ...
    ctx.translate(
      config.x - (icon[0].w * scale) / 2,
      config.y - (icon[0].h * scale) / 2
    );
  ...
  diffOptionsIcon(config) {
    ...
    if (
      config.iconName === 'download' ||
      config.iconName === 'back' ||
      config.iconName === 'trash'
    ) {
  ...
  diffCursor(config) {
    ...
    this.layeredText(
      ...
      [{ outline: '#fff', letterBorder: 11 }, { fill: '#000' }]
    );
  ...
  category(config) {
    ...
    if (config.scale || config.right) {
      ctx.scale(
        (config.right ? -1 : 1) * (config.scale || 1),
        config.scale || 1
      );
    }
  ...
  nameplate(config) {
    ...
    this.layeredText(
      ...
      [{ outline: '#000', letterBorder: 6 }, { fill: '#fff' }]
    );
