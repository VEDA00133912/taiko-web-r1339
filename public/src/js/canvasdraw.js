class CanvasDraw{
	...
	gauge(config){
		...
		if (config.percentage === 1) {
			...
			let colors = [
				"#f46584", "#f79a70", "#e6e071", "#8ef491", "#60eeb0", 
				"#58dae0", "#55afed", "#9233f6"
			];
			...
			if (this.frameCount % 5 === 0) { 
				this.animationFrameIndex = (this.animationFrameIndex + 1) % colors.length;
			}
			...
			for (let i = 0; i < colors.length; i++) {
				gradient.addColorStop(1 - (i / (colors.length - 1)), colors[(i + this.animationFrameIndex) % colors.length]);
			}
			...
			if(gaugeFilled > gaugeClear + 14){
				...
				for (let i = 0; i < colors.length; i++) {
					yellowGradient.addColorStop(1 - (i / (colors.length - 1)), colors[(i + this.animationFrameIndex) % colors.length]); 
				}
				...
				glossGradient.addColorStop(0, colors[(this.animationFrameIndex) % colors.length]);  
				glossGradient.addColorStop(1, colors[(this.animationFrameIndex + 1) % colors.length]);  
