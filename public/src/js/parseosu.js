class ParseOsu {
  ...
  parseTiming() {
    ...
    for (var i = indexes.start; i <= indexes.end; i++) {
      ...
      if (i == indexes.start) {
        ...
        this.beatInfo.bpm = Math.floor(
          (1000 / this.beatInfo.beatInterval) * 60
        );
      }
      var beatReset = false;
      if (msOrPercent < 0) {
        var sliderMultiplier =
          this.difficulty.lastMultiplier / Math.abs(msOrPercent / 100);
      } else {
  ...
  parseMeasures() {
    ...
    for (var i = 0; i < this.timingPoints.length; i++) {
      ...
      var limit =
        this.circles[this.circles.length - 1].endTime + currentTiming.beatMS;

      for (var j = i + 1; j < this.timingPoints.length; j++) {
        var nextTiming = this.timingPoints[j];
        var newLimit = nextTiming.start;
        if (
          nextTiming.measure !== currentTiming.measure ||
          nextTiming.beatReset
        ) {
  ...
  parseCircles() {
    ...
    for (var i = indexes.start; i <= indexes.end; i++) {
      ...
      if (osuType & this.osu.SPINNER) {
        ...
        var hitMultiplier =
          this.difficultyRange(this.difficulty.overallDifficulty, 3, 5, 7.5) *
          1.65;
        var requiredHits = Math.floor(
          Math.max(1, ((endTime - start) / 1000) * hitMultiplier)
        );
        pushCircle(
          new Circle({
            id: circleID,
        ...
        var distance =
          parseFloat(extras[this.osu.PIXELLENGTH]) *
          parseFloat(extras[this.osu.REPEAT]);
        ...
        if (hitSound & this.osu.FINISH) {
          if (hitSound & this.osu.WHISTLE || hitSound & this.osu.CLAP) {
            ...
            hitSound & this.osu.NORMAL ||
            hitSound === this.osu.FINISH
          ) {
            type = 'daiDon';
      ...
      if (emptyValue) {
        console.warn(
          'Unknown note type found on line ' + (i + 1) + ': ' + this.data[i]
        );
      }
    }
    this.scoremode = 2;
