    ...
    try {
      ...
      for (var i in this.items) {
        ...
        if (current.type === 'language') {
          ...
          if (
            (current.type === 'select' || current.type === 'gamepad') &&
            current.options.indexOf(storage[i]) === -1
          ) {
            this.storage[i] = null;
  ...
  addLang(lang, forceSet) {
    ...
    if (lang.categories) {
      assets.categories.forEach((category) => {
        if (
          'title_lang' in category &&
          lang.categories[category.title_lang.en]
        ) {
          category.title_lang[lang.id] =
            lang.categories[category.title_lang.en];
        }
      });
    }
...
class SettingsView {
  ...
  init(touchEnabled, tutorial, songId, toSetting, settingsItems, noSoundStart) {
    ...
    pageEvents.add(
      window,
      ['mouseup', 'touchstart', 'touchmove', 'touchend', 'blur'],
      (event) => {
        ...
        if (event.type === 'touchstart') {
          ...
          var cursor = event.changedTouches[0];
          if (
            Math.abs(move.x - cursor.pageX) > 10 ||
            Math.abs(move.y - cursor.pageY) > 10
          ) {
            move.active = true;
          }
        } else {
    ...
    for (let i in this.settingsItems) {
      ...
      if (current.indent) {
        settingBox.style.marginLeft =
          (2 * current.indent || 0).toString() + 'em';
      }
      ...
      if (current.description || current.description_lang) {
        settingBox.title =
          this.getLocalTitle(current.description, current.description_lang) ||
          '';
      }
      ...
      if (
        (!toSetting && this.items.length === this.selected) ||
        toSetting === i
      ) {
    ...
    if (this.customSettings) {
      ...
      this.browseButton.classList.add(
        'taibtn',
        'stroke-sub',
      ...
      this.defaultButton.parentNode.insertBefore(
        this.browseButton,
        this.defaultButton
      );
      this.items.push({
    ...
    this.showDefault =
      !this.customSettings ||
      plugins.allPlugins.filter((obj) => obj.plugin.imported).length;
    if (this.showDefault) {
      this.items.push({
        id: 'default',
    ...
    if (!this.customSettings) {
      ...
      this.gamepadTitle =
        this.gamepadSettings.getElementsByClassName('view-title')[0];
      this.gamepadEndButton =
        this.gamepadSettings.getElementsByClassName('view-end-button')[0];
      this.addTouch(this.gamepadEndButton, (event) => this.gamepadBack(true));
      this.gamepadBox =
        this.gamepadSettings.getElementsByClassName('setting-box')[0];
      ...
      this.latencyTitle =
        this.latencySettings.getElementsByClassName('view-title')[0];
      this.latencyItems = [];
      this.latencySelected = 0;
      var latencyContent =
        this.latencySettings.getElementsByClassName('view-content')[0];
      ...
      this.addTouch(this.latencyDefaultButton, (event) =>
        this.latencyDefault()
      );
      this.latencyEndButton =
        this.latencySettings.getElementsByClassName('view-end-button')[0];
      this.latencyItems.push({
        id: 'back',
  ...
  getValue(name, valueDiv) {
    ...
    if (current.type === 'language') {
      ...
      if (current.format || current.format_lang) {
        value = this.getLocalTitle(current.format, current.format_lang).replace(
          '%s',
          value
        );
      }
  ...
  setValue(name) {
    ...
    if (current.type === 'language' || current.type === 'select') {
      value =
        current.options[
          this.mod(current.options.length, current.options.indexOf(value) + 1)
        ];
    } else if (current.type === 'toggle') {
      value = !value;
    } else if (current.type === 'keyboard') {
  ...
  keyPressed(pressed, name, event, repeat) {
    ...
    if (this.mode === 'settings') {
      if (name === 'confirm') {
        ...
        do {
          this.selected = this.mod(
            this.items.length,
            this.selected + (name === 'right' || name === 'down' ? 1 : -1)
          );
        } while (
          (this.items[this.selected].id === 'default' ||
            this.items[this.selected].id === 'browse') &&
          name !== 'left'
        );
      ...
      if (name === 'back') {
        ...
        this.playSound(
          current === 'ka_l' || current === 'ka_r' ? 'se_ka' : 'se_don'
        );
      ...
      if (name === 'confirm') {
        ...
        do {
          this.latencySelected = this.mod(
            this.latencyItems.length,
            this.latencySelected +
              (name === 'right' || name === 'down' ? 1 : -1)
          );
        } while (
          this.latencyItems[this.latencySelected].id === 'default' &&
          name !== 'left'
        );
      ...
      if (name === 'confirm' || name === 'back') {
        ...
        this.latencySetAdjust(
          latencySelected,
          name === 'up' || name === 'right' ? 1 : -1
        );
        if (event) {
          event.preventDefault();
        }
  ...
  gamepadSet(diff) {
    ...
    var current = this.settingsItems[selected.id];
    if (diff) {
      this.gamepadSelected = this.mod(
        current.options.length,
        this.gamepadSelected + diff
      );
      this.playSound('se_ka');
    }
    ...
    this.gamepadButtons.style.backgroundPosition =
      '0 ' + (-11.87 - 4.93 * this.gamepadSelected) + 'em';
  ...
  latencyGetValue(name, valueText) {
    ...
    if (name === 'drumSounds') {
      valueText.data = currentLatency[name]
        ? strings.settings.on
  ...
  latencySetValue(name, touched) {
    ...
    if (name === 'calibration') {
      ...
      if (this.latencySelected !== selectedIndex) {
        this.latencyItems[this.latencySelected].settingBox.classList.remove(
          'selected'
        );
  ...
  numberAdjust(selected, add) {
    ...
    var valueText = Intl.NumberFormat(
      strings.intl,
      selected.sign
        ? {
            signDisplay: 'always',
        ...
        : undefined
    ).format(selectedItem.value / mul);
    if (selected.format || selected.format_lang) {
      valueText = this.getLocalTitle(
        selected.format,
        selected.format_lang
      ).replace('%s', valueText);
    }
  ...
  onEnd() {
    ...
    setTimeout(() => {
      if (this.tutorial && !this.touched) {
        ...
        new SongSelect(
          this.tutorial ? false : this.customSettings ? 'plugins' : 'settings',
          false,
          this.touched,
          this.songId
        );
  ...
  restart() {
    ...
    return new SettingsView(
      ...
      this.songId,
      undefined,
      this.customSettings ? plugins.getSettings() : undefined,
      true
    );
  }
    ...
    for (var i in this.items) {
      ...
      if (item.valueDiv) {
        ...
        if (item.description || item.description_lang) {
          item.settingBox.title =
            this.getLocalTitle(item.description, item.description_lang) || '';
        }
  ...
  setStrings() {
    this.setAltText(
      this.viewTitle,
      this.customSettings ? strings.plugins.title : strings.gameSettings
    );
    ...
    if (this.showDefault) {
      this.setAltText(
        this.defaultButton,
        this.customSettings
          ? strings.plugins.unloadAll
          : strings.settings.default
      );
    }
  }
  ...
  clean(noSoundStop) {
    ...
    if (this.resolution !== settings.getItem('resolution')) {
      for (var i in assets.image) {
        if (
          i === 'touch_drum' ||
          i.startsWith('bg_song_') ||
          i.startsWith('bg_stage_') ||
          i.startsWith('bg_don_') ||
