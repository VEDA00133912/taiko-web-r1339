  ...
  strReplace(input, searchString, insertedText, repeat = 1) {
    ...
    for (var i = 0; i < repeat; i++) {
      ...
      input =
        input.slice(0, index) +
        insertedText +
        input.slice(index + searchString.length);
      ...
      if (!plugin.hide) {
        ...
        if (module) {
          description = [
            module.description,
            module.author
              ? strings.plugins.author.replace('%s', module.author)
              : null,
            module.version
              ? strings.plugins.version.replace('%s', module.version)
              : null,
          ]
            .filter(Boolean)
            .join('\n');
          ...
          languageList.forEach((lang) => {
            description_lang[lang] = [
              this.getLocalTitle(
                module.description,
                module.description_lang,
                lang
              ),
              module.author
                ? allStrings[lang].plugins.author.replace('%s', module.author)
                : null,
              module.version
                ? allStrings[lang].plugins.version.replace('%s', module.version)
                : null,
            ]
              .filter(Boolean)
              .join('\n');
      ...
      return (
        ...
        (module) => {
          ...
          try {
            ...
            var error = new Error();
            error.stack =
              'Error initializing plugin: ' + this.name + '\n' + e.stack;
            if (loadErrors) {
        ...
        (e) => {
          ...
          if (e.name === 'SyntaxError') {
            ...
            error.stack =
              'Error in plugin syntax: ' + this.name + '\n' + e.stack;
          } else {
            var error = e;
          }
          if (loadErrors) {
...
class EditValue {
  ...
  start() {
    ...
    try {
      ...
      var error = new Error();
      error.stack =
        'Error editing the value of ' + this.getName() + '\n' + e.stack;
      throw error;
    }
    if (typeof output === 'undefined') {
      console.error(this.loadCallback);
      throw new Error(
        'Error editing the value of ' +
          this.getName() +
      ...
      if (this.name) {
        var name =
          ((typeof this.name[0] === 'function' && this.name[0].name) ||
            (typeof this.name[0] === 'object' &&
              typeof this.name[0].constructor === 'function' &&
              (this.name[0] instanceof this.name[0].constructor
                ? (() => {
                    var consName = this.name[0].constructor.name || '';
                    return (
                      consName.slice(0, 1).toLowerCase() + consName.slice(1)
                    );
                  })()
                : this.name[0].constructor.name + '.prototype')) ||
            name) + (this.name[1] ? '.' + this.name[1] : '');
      }
    } catch (e) {
      name = 'error';
    }
...
class EditFunction extends EditValue {
  start() {
    ...
    if (typeof this.original !== 'function') {
      ...
      var error = new Error();
      error.stack =
        'Error editing the function value of ' +
        this.getName() +
        ': Original value is not a function';
      throw error;
    }
    ...
    try {
      ...
      error.stack =
        'Error editing the function value of ' +
        this.getName() +
        ': Could not evaluate string, check the full string for errors: ' +
        url +
        '\n' +
        e.stack;
      setTimeout(
