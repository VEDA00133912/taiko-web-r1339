class Search {
      ...
      if ('id' in skin || i === 'default') {
        ...
        css.push(
          loader.cssRuleset({
            ...
            ['.song-search-' +
            id +
            ' .song-search-result-title::before, .song-search-' +
            id +
      ...
      Object.keys(filters).forEach((filter) => {
        ...
        switch (filter) {
          ...
          case 'ura':
            if (
              song.courses[filter] &&
              song.courses[filter].stars >= value.min &&
              song.courses[filter].stars <= value.max
            ) {
              passedFilters++;
            }
            break;
            ...
            if (value === 'any') {
              ...
              scoreStorage.difficulty.forEach((difficulty) => {
                if (
                  score &&
                  score[difficulty] &&
                  score[difficulty].crown &&
                  (filter === 'clear' || score[difficulty].crown === filter)
                ) {
                  passedFilters++;
                }
              });
              ...
              if (
                ...
                score[value] &&
                score[value].crown &&
                (filter === 'clear' || score[value].crown === filter)
              ) {
                passedFilters++;
              }
            }
          ...
          case 'maker':
            if (
              song.maker &&
              song.maker.name.toLowerCase().includes(value.toLowerCase())
            ) {
              passedFilters++;
            }
            break;
          case 'genre':
            var cat = assets.categories.find(
              (cat) => cat.id === song.category_id
            );
            var aliases = cat.aliases
              ? cat.aliases.concat([cat.title])
              : [cat.title];
            ...
            break;
          case 'diverge':
            var branch = Object.values(song.courses).find(
              (course) => course && course.branch
            );
  ...
  createResult(result, resultWidth, fontSize) {
    ...
    var subtitle = this.songSelect.getLocalTitle(
    ...
    ranges.forEach((range) => {
      if (lastIdx !== range[0]) {
        fragment.appendChild(
          document.createTextNode(text.slice(lastIdx, range[0]))
        );
      }
  ...
  remove(byUser = false) {
    if (this.opened) {
      ...
      pageEvents.remove(
        this.div.querySelector(':scope #song-search-container'),
        ['mousedown', 'touchstart']
      );
  ...
  setTip(tip, error = false) {
    ...
    if (!tip) {
      tip =
        ...
        ' ' +
        strings.search.tips[
          Math.floor(Math.random() * strings.search.tips.length)
        ];
    }
  ...
  proceed(songId) {
    ...
    var songIndex = this.songSelect.songs.findIndex(
      (song) => song.id === songId
    );
  ...
  onInput(resize) {
    ...
    var fontSize = parseFloat(
      getComputedStyle(
        this.div.querySelector(':scope #song-search')
      ).fontSize.slice(0, -2)
    );
    var resultsWidth = parseFloat(
      getComputedStyle(resultsDiv).width.slice(0, -2)
    );
    ...
    var resultWidth =
      ...
      1.8 * fontSize -
      0.8 * fontSize -
      (courseWidth + 0.4 * fontSize * 1.2) * 5 -
      0.6 * fontSize;
  ...
  onClick(e) {
    if (
      (e.target.id === 'song-search-container' ||
        e.target.id === 'song-search-close') &&
      e.which === 1
    ) {
      this.remove(true);
    } else if (e.which === 1) {
  ...
  keyPress(pressed, name, event, repeat, ctrl) {
    if (
      name === 'back' ||
      (event && event.keyCode && event.keyCode === 70 && ctrl)
    ) {
      this.remove(true);
      if (event) {
